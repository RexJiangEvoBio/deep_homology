...
# Regulators' effects on phenotypic traits
effect_all<-read.table("data_effect_all.txt",sep="\t") # Read previously generate data file
effect_all=as.matrix(effect_all) # Convert to a matrix
n_regulator=ncol(effect_all);n_dim=nrow(effect_all) # Number of regulators and traits saved as scalars

# Set ancestral state
X_ances=rep(1,n_regulator) # An ancestral state where everything is lowly expressed
z_ances=effect_all%*%X_ances # Calculate ancestral phenotypic state

# Traits to be under selection
n_selection=5 # The number of phenotypic dimensions under selection
sub_selection=sort(sample(1:n_dim,n_selection)) # Randomly draw a set of traits subject to selection
